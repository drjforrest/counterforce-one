...
def analyze_community_responses():
    ...
    for post in session.query(RedditPost).all():
        comment_count = (
            session.query(RedditComment).filter_by(post_id=post.post_id).count()
        )
        if post.subreddit not in subreddit_comments:
            subreddit_comments[post.subreddit] = {
                "posts": 0,
                "comments": 0,
                "total_score": 0,
            }
    ...
    for subreddit, data in sorted(
        subreddit_comments.items(), key=lambda x: x[1]["comments"], reverse=True
    ):
        avg_comments = data["comments"] / data["posts"] if data["posts"] > 0 else 0
        print(
            f"  r/{subreddit}: {data['comments']} comments across {data['posts']} posts (avg: {avg_comments:.1f})"
        )
    ...
    for comment in (
        session.query(RedditComment).limit(1000).all()
    ...
    health_percentage = (
        (health_comments / total_analyzed * 100) if total_analyzed > 0 else 0
    )
    print(
        f"  Health-related comments: {health_comments}/{total_analyzed} ({health_percentage:.1f}%)"
    )
    ...
    for comment in session.query(RedditComment).limit(2000).all():
        comment_text = comment.body.lower()
        if (
            any(keyword in comment_text for keyword in health_keywords)
            and len(comment.body) > 50
        ):
